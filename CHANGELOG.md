...
- Recalculated 3,136 compact-metric risk rows with maximum absolute error below \(1.4 \times 10^{-11}\); verified 672 quality rows, 12,544 risk rows, 8,960 DRUNet calls and 336 FBCNN calls.
- Verified both executed notebooks had seven sequentially executed code cells, zero errors and only their intended `SHARD_INDEX` source changes.
- Recorded outer-ZIP mismatches for both downloaded archives while every internal manifest hash and byte count matched; accepted both scientific payloads with the container findings preserved.
- Advanced the development registry to 9 of 12. The shard 09 executed notebook is verified, but its archive is still required before the registry can advance. Independent test inference remains unauthorized and unperformed.

## 0.23.6 — 2026-09-24

- Independently validated development shard 06 of 12 for DIV2K `0853`–`0860`: all 124 manifested files, 56 records and compact bundles passed integrity and schema checks.
- Recalculated 1,568 compact-metric risk rows with maximum absolute error below \(8.4 \times 10^{-12}\); verified 336 quality rows, 6,272 risk rows, 4,480 DRUNet calls and 168 FBCNN calls.
- Verified that the renamed executed notebook had seven sequentially executed code cells, zero errors and only the intended `SHARD_INDEX = 6` source change.
- Recorded that the downloaded outer ZIP SHA-256 differs from the notebook-reported SHA-256 while every internal manifest hash and byte count matches; accepted the scientific payload with the container finding preserved.
- Advanced the development registry to 7 of 12 and the next permitted action to shard index 7. Independent test inference remains unauthorized and unperformed.

## 0.23.5 — 2026-09-24

- Independently validated development shard 05 of 12 for DIV2K `0845`–`0852`: all 124 manifested files, 56 records and compact bundles passed integrity and schema checks.
- Recalculated 1,568 compact-metric risk rows with maximum absolute error below \(1.4 \times 10^{-11}\); verified 336 quality rows, 6,272 risk rows, 4,480 DRUNet calls and 168 FBCNN calls.
- Verified the executed notebook had seven sequentially executed code cells, zero errors and only the intended `SHARD_INDEX = 5` source change.
- Recorded that the downloaded outer ZIP SHA-256 differs from the notebook-reported SHA-256 while every internal manifest hash and byte count matches; accepted the scientific payload with the container finding preserved.
- Advanced the development registry to 6 of 12 and the next permitted action to shard index 6. Independent test inference remains unauthorized and unperformed.

## 0.23.4 — 2026-09-24

- Independently validated development shard 04 of 12 for DIV2K `0837`–`0844`: all 124 manifested files, 56 records and compact bundles passed integrity and schema checks.
- Recalculated 1,568 compact-metric risk rows with maximum absolute error below \(5.8 \times 10^{-12}\); verified 336 quality rows, 6,272 risk rows, 4,480 DRUNet calls and 168 FBCNN calls.
- Verified the executed notebook had seven sequentially executed code cells, zero errors and only the intended `SHARD_INDEX = 4` source change.
- Recorded that the downloaded outer ZIP SHA-256 differs from the notebook-reported SHA-256 while every internal manifest hash and byte count matches; accepted the scientific payload with the container finding preserved.
- Advanced the development registry to 5 of 12 and the next permitted action to shard index 5. Independent test inference remains unauthorized and unperformed.
...
- Recalculated 1,568 compact-metric risk rows with maximum absolute error below \(3.1 \times 10^{-11}\); verified 336 quality rows, 6,272 risk rows, 4,480 DRUNet calls and 168 FBCNN calls.
- Recorded that the downloaded outer ZIP SHA-256 differs from the executed notebook's reported SHA-256 while every internal manifest hash and byte count matches; accepted the scientific payload with the container finding preserved.
- Advanced the development registry to 3 of 12 and the next permitted action to shard index 3. Independent test inference remains unauthorized and unperformed.

## 0.23.1 — 2026-09-24

- Independently validated development shard 01 of 12 for DIV2K `0813`–`0820`: all 124 manifested files, 56 records and compact bundles passed integrity and schema checks.
- Recalculated 1,568 compact-metric risk rows with maximum absolute error below (4 \\times 10^{-12}); verified 336 quality rows, 6,272 risk rows, 4,480 DRUNet calls and 168 FBCNN calls.
- Recorded that the downloaded outer ZIP SHA-256 differs from the executed notebook's reported SHA-256 while every internal manifest hash and byte count matches; accepted the scientific payload with the container finding preserved.
- Advanced the development registry to 2 of 12 and the next permitted action to shard index 2. Independent test inference remains unauthorized and unperformed.
