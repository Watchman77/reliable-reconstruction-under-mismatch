// One-time v0.15 -> v0.16 migration; preserves seed IDs, historic triage and formal decisions.
...
if(!repo||!out)throw new Error('Usage: node update_full_text_09.mjs REPO OUTPUT_DIR');
await fs.mkdir(out,{recursive:true});
const batch=JSON.parse(await fs.readFile(path.join(repo,'literature/screening/full_text_09.json'),'utf8'));
const input=path.join(repo,'literature/evidence_matrix_2020_2026.xlsx');
const bytes=await fs.readFile(input);
const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
assert.equal(sha,'1d91ec8975fb13c46a62ac0123c653c6900c4305','Requires v0.15; must not overwrite a later checkpoint.');
...
for(const r of batch.records){
 ...
 ref['Review notes']+=` | AI full-text ${batch.checked_on}: ${r.decision}; human pending. Verified fields: ${Object.keys(r.feature_codes).join(', ')}. Evidence/version/read extent: screening/full_text_09.json (${r.study_id}). Unlisted feature codes remain provisional.`;
...
for(const r of batch.pending_access){
 ...
 assert(i>=0);assert.equal(matrix.getRange(`A${row}`).values[0][0],r.study_id);
 assert.equal(log.getRange(`J${row}`).values[0][0],'Pending');
 ref['Review notes']+=` | Retrieval ${batch.checked_on}: full-text eligibility pending. Source attempts and next action: screening/full_text_09.json (${r.study_id}, pending_access). No new feature verification.`;
 set(matrix,`AF${row}`,ref['Review notes'],'Document access attempts; eligibility unchanged.');
 set(log,`L${row}`,`${batch.checked_on} | Full-text eligibility remains pending. ${r.attempts[0].result} ${r.next_action} Complete retrieval log: screening/full_text_09.json (${r.study_id}).`, 'Document unresolved access without an eligibility decision.');
 log.getRange(`A${row}:N${row}`).format.rowHeight=144;
 log.getRange(`J${row}:N${row}`).format.wrapText=true;
 log.getRange(`A${row}:N${row}`).format.verticalAlignment='top';
}
set(summary,'A17','Joint PSF estimation, compound night restoration, cross-camera video transfer and clean-class stopping have comparators. Six full texts remain pending. See screening/full_text_09_report.md for assumptions and version-specific limits. Earlier checkpoints remain available. Novelty is unestablished.');
set(protocol,'B22','Initial triage 90/90 (82 advance, 8 unclear). AI full-text assessed: 84 (83 include, 1 exclude), 6 remain. All human adjudication and formal searches/inclusion pending. Formal inclusion/exclusion counts remain zero.');
set(protocol,'B23','90 seed records retained. Cumulative full-text progress: screening/full_text_01.json, full_text_02.json, full_text_03.json, full_text_04.json, full_text_05.json, full_text_06.json, full_text_07.json, full_text_08.json and full_text_09.json; current report: full_text_09_report.md. AI decisions require human adjudication. Only listed feature fields are verified; other codes remain provisional.');
wb.recalculate();
assert.deepEqual(summary.getRange('E11:E14').values.flat(),[90,82,8,0]);
assert.equal(summary.getRange('E15').values[0][0],84);
const decision=log.getRange('J74').values[0][0];log.getRange('J74').values=[['Pending']];
assert.equal(summary.getRange('E15').values[0][0],83);
log.getRange('J74').values=[[decision]];wb.recalculate();
assert.equal(summary.getRange('E15').values[0][0],84);
assert.deepEqual(summary.getRange('B7:B12').values.flat(),[0,0,0,0,0,0]);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='Pending').length,6);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='AI full-text: include').length,83);
...
for(const [name,range] of [['Summary','A5:H29'],['Screening Log','I74:N74'],['Evidence Matrix','O74:X74'],['Review Protocol','A21:B25']]){
...
const audit={checked_on:batch.checked_on,baseline_workbook_blob:sha,note:'4 additional AI full-text decisions and 6 pending retrieval logs; cumulative 84 assessed, 83 include, 1 exclude, 6 pending. Historical triage and formal decisions preserved.',changes,validation:{...batch.counts,batch_counts:batch.batch_counts,initial_triage:[90,82,8,0],formula_input_change_test:'passed',unintended_cell_changes:unintended,formula_errors:errors}};
await fs.writeFile(path.join(out,'full_text_09_changes.json'),JSON.stringify(audit,null,2)+'\n');
