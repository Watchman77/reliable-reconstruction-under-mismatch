// One-time v0.9 -> v0.10 migration; preserves seed IDs, historic triage and formal decisions.
...
if(!repo||!out)throw new Error('Usage: node update_full_text_03.mjs REPO OUTPUT_DIR');
await fs.mkdir(out,{recursive:true});
const batch=JSON.parse(await fs.readFile(path.join(repo,'literature/screening/full_text_03.json'),'utf8'));
const input=path.join(repo,'literature/evidence_matrix_2020_2026.xlsx');
const bytes=await fs.readFile(input);
const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
assert.equal(sha,'765f5500a12b7d1b5321032f45d0e982a490bb69','Requires v0.9; must not overwrite a later checkpoint.');
...
for(const r of batch.records){
 ...
 ref['Review notes']+=` | AI full-text ${batch.checked_on}: ${r.decision}; human pending. Verified fields: ${Object.keys(r.feature_codes).join(', ')}. Evidence/version/read extent: screening/full_text_03.json (${r.study_id}). Unlisted feature codes remain provisional.`;
 ...
 set(log,`L${row}`,`${batch.checked_on} | AI full-text ${r.decision}; human pending. ${r.decision==='exclude'?r.exclusion_detail:r.criteria['3'].evidence} Online text inspected; no local PDF saved. Version, read extent and technical extraction: screening/full_text_03.json (${r.study_id}).`);
...
set(summary,'A17','Joint sampling, operator point estimates, compound restoration and image coverage have existing comparators. See screening/full_text_03_report.md for assumptions and version-specific limits. Earlier checkpoints remain available. Novelty is unestablished.');
set(protocol,'B22','Initial triage 90/90 (82 advance, 8 unclear). AI full-text assessed: 36 (35 include, 1 exclude), 54 remain. All human adjudication and formal searches/inclusion pending. Formal inclusion/exclusion counts remain zero.');
set(protocol,'B23','90 seed records retained. Cumulative full-text progress: screening/full_text_01.json, full_text_02.json and full_text_03.json; current report: full_text_03_report.md. AI decisions require human adjudication. Only listed feature fields are verified; other codes remain provisional.');
wb.recalculate();
assert.deepEqual(summary.getRange('E11:E14').values.flat(),[90,82,8,0]);
assert.equal(summary.getRange('E15').values[0][0],36);
const decision=log.getRange('J8').values[0][0];log.getRange('J8').values=[['Pending']];
assert.equal(summary.getRange('E15').values[0][0],35);
log.getRange('J8').values=[[decision]];wb.recalculate();
assert.equal(summary.getRange('E15').values[0][0],36);
assert.deepEqual(summary.getRange('B7:B12').values.flat(),[0,0,0,0,0,0]);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='Pending').length,54);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='AI full-text: include').length,35);
...
for(const [name,range] of [['Summary','A5:H29'],['Screening Log','I8:N8'],['Evidence Matrix','O42:X42'],['Review Protocol','A21:B25']]){
