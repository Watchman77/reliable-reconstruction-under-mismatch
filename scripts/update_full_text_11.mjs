// One-time v0.17 -> v0.18 migration; preserves seed IDs, historic triage and formal decisions.
...
if(!repo||!out)throw new Error('Usage: node update_full_text_11.mjs REPO OUTPUT_DIR');
await fs.mkdir(out,{recursive:true});
const batch=JSON.parse(await fs.readFile(path.join(repo,'literature/screening/full_text_11.json'),'utf8'));
const input=path.join(repo,'literature/evidence_matrix_2020_2026.xlsx');
const bytes=await fs.readFile(input);
const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
assert.equal(sha,'edd64e1d0245a94391852f7a667347363623cee5','Requires v0.17; must not overwrite a later checkpoint.');
...
assert.equal(refs.length,90); assert.equal(batch.records.length,4);
const snapshots=[];
...
for(const r of batch.records){
 ...
 if(r.verified_title){
  ref.Title=r.verified_title;
  set(matrix,`D${row}`,r.verified_title,`${r.study_id}: full title verified in official proceedings; ${r.source_url}`);
  set(log,`E${row}`,r.verified_title,`${r.study_id}: synchronize verified publication title.`);
 }
 const status='AI full-text eligibility assessed; selected features verified';
 ref['Evidence status']=status;
 ref['Review notes']+=` | AI full-text ${batch.checked_on}: ${r.decision}; human pending. Verified fields: ${Object.keys(r.feature_codes).join(', ')}. Evidence/version/read extent: screening/full_text_11.json (${r.study_id}). Unlisted feature codes remain provisional.`;
 ...
 set(log,`L${row}`,`${batch.checked_on} | AI full-text ${r.decision}; human pending. ${r.decision==='exclude'?r.exclusion_detail:r.criteria['3'].evidence} ${r.local_pdf_acquired ? 'Uploaded PDF verified and inspected; source/version/hash in register; not committed.' : 'Online full text inspected; no local PDF acquired.'} Version, read extent and technical extraction: screening/full_text_11.json (${r.study_id}).`);
...
for(const r of batch.pending_access){
 const i=refs.findIndex(x=>x['Study ID']===r.study_id),row=i+2,ref=refs[i];
 assert(i>=0);assert.equal(matrix.getRange(`A${row}`).values[0][0],r.study_id);
 assert.equal(log.getRange(`J${row}`).values[0][0],'Pending');
 ref['Review notes']+=` | Retrieval ${batch.checked_on}: full-text eligibility pending. Source attempts and next action: screening/full_text_11.json (${r.study_id}, pending_access). No new feature verification.`;
 set(matrix,`AF${row}`,ref['Review notes'],'Document access attempts; eligibility unchanged.');
 set(log,`L${row}`,`${batch.checked_on} | Full-text eligibility remains pending. ${r.attempts[0].result} ${r.next_action} Complete retrieval log: screening/full_text_11.json (${r.study_id}).`, 'Document unresolved access without an eligibility decision.');
 log.getRange(`A${row}:N${row}`).format.rowHeight=144;
 log.getRange(`J${row}:N${row}`).format.wrapText=true;
 log.getRange(`A${row}:N${row}`).format.verticalAlignment='top';
}
for(const item of batch.lineage_updates??[]){
 for(const id of item.study_ids){
  const i=refs.findIndex(x=>x['Study ID']===id), row=i+2, ref=refs[i];
  assert(i>=0); assert.equal(matrix.getRange(`A${row}`).values[0][0],id);
  ref['Review notes']+=` | Lineage ${batch.checked_on}: P053 explicitly identifies P026 as previous work and comparator. Relationship confirmed; study-level grouping pending. See screening/full_text_11.json (lineage_updates).`;
  set(matrix,`AF${row}`,ref['Review notes'],'Link predecessor evidence without changing formal study grouping or decisions.');
 }
}
set(summary,'A17','VDPS confirms blind time-varying physical-parameter estimation. FaverNet follows AverNet; image sampling and variance maps do not establish calibration. Only P035 full text remains pending. See screening/full_text_11_report.md for assumptions and version-specific limits. Earlier checkpoints remain available. Novelty is unestablished.');
set(protocol,'B22','Initial triage 90/90 (82 advance, 8 unclear). AI full-text assessed: 89 (88 include, 1 exclude), 1 remains. All human adjudication and formal searches/inclusion pending. Formal inclusion/exclusion counts remain zero.');
set(protocol,'B23','90 seed records retained. Cumulative evidence: screening/full_text_01.json through full_text_11.json. Current report: full_text_11_report.md. AI decisions require human adjudication. Only listed feature fields are verified; other codes remain provisional.');
const oldStatusFormula=summary.getRange('A22').formulas[0][0];
const newStatusFormula=oldStatusFormula.replace('&" remain. Human','&IF(B6-E15=1," remains."," remain.")&" Human');
assert.notEqual(newStatusFormula,oldStatusFormula);
summary.getRange('A22').formulas=[[newStatusFormula]];
changes.push({sheet:'Summary',cell:'A22',old_formula:oldStatusFormula,new_formula:newStatusFormula,reason:'Use singular or plural wording for the live remaining-paper count.'});
wb.recalculate();
assert.deepEqual(summary.getRange('E11:E14').values.flat(),[90,82,8,0]);
assert.equal(summary.getRange('E15').values[0][0],89);
const decision=log.getRange('J31').values[0][0];log.getRange('J31').values=[['Pending']];
assert.equal(summary.getRange('E15').values[0][0],88);
assert(summary.getRange('A22').values[0][0].includes('2 remain.'));
log.getRange('J31').values=[[decision]];wb.recalculate();
assert.equal(summary.getRange('E15').values[0][0],89);
assert(summary.getRange('A22').values[0][0].includes('1 remains.'));
assert.deepEqual(summary.getRange('B7:B12').values.flat(),[0,0,0,0,0,0]);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='Pending').length,1);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='AI full-text: include').length,88);
...
assert.equal(batch.records.filter(r=>r.local_pdf_acquired).length,4);
...
for(const [name,range] of [['Summary','A5:H29'],['Screening Log','I31:N31'],['Evidence Matrix','O31:X31'],['Review Protocol','A21:B25']]){
...
const audit={checked_on:batch.checked_on,baseline_workbook_blob:sha,note:'4 additional AI full-text decisions, 1 pending retrieval log and a P026/P053 lineage note; cumulative 89 assessed, 88 include, 1 exclude, 1 pending. Historical triage and formal decisions preserved.',changes,validation:{...batch.counts,batch_counts:batch.batch_counts,initial_triage:[90,82,8,0],formula_input_change_test:'passed',unintended_cell_changes:unintended,formula_errors:errors}};
await fs.writeFile(path.join(out,'full_text_11_changes.json'),JSON.stringify(audit,null,2)+'\n');
