// One-time v0.16 -> v0.17 migration; preserves seed IDs, historic triage and formal decisions.
...
if(!repo||!out)throw new Error('Usage: node update_full_text_10.mjs REPO OUTPUT_DIR');
await fs.mkdir(out,{recursive:true});
const batch=JSON.parse(await fs.readFile(path.join(repo,'literature/screening/full_text_10.json'),'utf8'));
const input=path.join(repo,'literature/evidence_matrix_2020_2026.xlsx');
const bytes=await fs.readFile(input);
const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
assert.equal(sha,'96879809216a0dec7762eabb35f5b4ecfe3133b3','Requires v0.16; must not overwrite a later checkpoint.');
...
for(const r of batch.records){
 ...
 ref['Review notes']+=` | AI full-text ${batch.checked_on}: ${r.decision}; human pending. Verified fields: ${Object.keys(r.feature_codes).join(', ')}. Evidence/version/read extent: screening/full_text_10.json (${r.study_id}). Unlisted feature codes remain provisional.`;
...
for(const r of batch.pending_access){
 ...
 assert(i>=0);assert.equal(matrix.getRange(`A${row}`).values[0][0],r.study_id);
 assert.equal(log.getRange(`J${row}`).values[0][0],'Pending');
 ref['Review notes']+=` | Retrieval ${batch.checked_on}: full-text eligibility pending. Source attempts and next action: screening/full_text_10.json (${r.study_id}, pending_access). No new feature verification.`;
 set(matrix,`AF${row}`,ref['Review notes'],'Document access attempts; eligibility unchanged.');
 set(log,`L${row}`,`${batch.checked_on} | Full-text eligibility remains pending. ${r.attempts[0].result} ${r.next_action} Complete retrieval log: screening/full_text_10.json (${r.study_id}).`, 'Document unresolved access without an eligibility decision.');
 log.getRange(`A${row}:N${row}`).format.rowHeight=144;
 log.getRange(`J${row}:N${row}`).format.wrapText=true;
 log.getRange(`A${row}:N${row}`).format.verticalAlignment='top';
}
set(summary,'A17','P029 adds a structured mismatch-correction comparator. Calibrated operator uncertainty and selective release are not demonstrated there. Five full texts remain pending. See screening/full_text_10_report.md for assumptions and version-specific limits. Earlier checkpoints remain available. Novelty is unestablished.');
set(protocol,'B22','Initial triage 90/90 (82 advance, 8 unclear). AI full-text assessed: 85 (84 include, 1 exclude), 5 remain. All human adjudication and formal searches/inclusion pending. Formal inclusion/exclusion counts remain zero.');
set(protocol,'B23','90 seed records retained. Cumulative full-text progress: screening/full_text_01.json, full_text_02.json, full_text_03.json, full_text_04.json, full_text_05.json, full_text_06.json, full_text_07.json, full_text_08.json, full_text_09.json and full_text_10.json; current report: full_text_10_report.md. AI decisions require human adjudication. Only listed feature fields are verified; other codes remain provisional.');
wb.recalculate();
assert.deepEqual(summary.getRange('E11:E14').values.flat(),[90,82,8,0]);
assert.equal(summary.getRange('E15').values[0][0],85);
const decision=log.getRange('J30').values[0][0];log.getRange('J30').values=[['Pending']];
assert.equal(summary.getRange('E15').values[0][0],84);
log.getRange('J30').values=[[decision]];wb.recalculate();
assert.equal(summary.getRange('E15').values[0][0],85);
assert.deepEqual(summary.getRange('B7:B12').values.flat(),[0,0,0,0,0,0]);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='Pending').length,5);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='AI full-text: include').length,84);
...
assert.equal(batch.records.filter(r=>r.local_pdf_acquired).length,0);
...
for(const [name,range] of [['Summary','A5:H29'],['Screening Log','I30:N30'],['Evidence Matrix','O30:X30'],['Review Protocol','A21:B25']]){
...
const audit={checked_on:batch.checked_on,baseline_workbook_blob:sha,note:'1 additional AI full-text decision and 5 pending retrieval logs; cumulative 85 assessed, 84 include, 1 exclude, 5 pending. Historical triage and formal decisions preserved.',changes,validation:{...batch.counts,batch_counts:batch.batch_counts,initial_triage:[90,82,8,0],formula_input_change_test:'passed',unintended_cell_changes:unintended,formula_errors:errors}};
await fs.writeFile(path.join(out,'full_text_10_changes.json'),JSON.stringify(audit,null,2)+'\n');
