// One-time v0.14 -> v0.15 migration; preserves seed IDs, historic triage and formal decisions.
...
if(!repo||!out)throw new Error('Usage: node update_full_text_08.mjs REPO OUTPUT_DIR');
await fs.mkdir(out,{recursive:true});
const batch=JSON.parse(await fs.readFile(path.join(repo,'literature/screening/full_text_08.json'),'utf8'));
const input=path.join(repo,'literature/evidence_matrix_2020_2026.xlsx');
const bytes=await fs.readFile(input);
const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
assert.equal(sha,'dc7d4c9a9f090a921844a2c466a982c9fa3ed42c','Requires v0.14; must not overwrite a later checkpoint.');
...
for(const r of batch.records){
 ...
 ref['Review notes']+=` | AI full-text ${batch.checked_on}: ${r.decision}; human pending. Verified fields: ${Object.keys(r.feature_codes).join(', ')}. Evidence/version/read extent: screening/full_text_08.json (${r.study_id}). Unlisted feature codes remain provisional.`;
...
set(summary,'A17','Operator-family training, blind calibration, degradation classification and video fidelity controls have existing comparators. See screening/full_text_08_report.md for assumptions and version-specific limits. Earlier checkpoints remain available. Novelty is unestablished.');
set(protocol,'B22','Initial triage 90/90 (82 advance, 8 unclear). AI full-text assessed: 80 (79 include, 1 exclude), 10 remain. All human adjudication and formal searches/inclusion pending. Formal inclusion/exclusion counts remain zero.');
set(protocol,'B23','90 seed records retained. Cumulative full-text progress: screening/full_text_01.json, full_text_02.json, full_text_03.json, full_text_04.json, full_text_05.json, full_text_06.json, full_text_07.json and full_text_08.json; current report: full_text_08_report.md. AI decisions require human adjudication. Only listed feature fields are verified; other codes remain provisional.');
wb.recalculate();
assert.deepEqual(summary.getRange('E11:E14').values.flat(),[90,82,8,0]);
assert.equal(summary.getRange('E15').values[0][0],80);
const decision=log.getRange('J47').values[0][0];log.getRange('J47').values=[['Pending']];
assert.equal(summary.getRange('E15').values[0][0],79);
log.getRange('J47').values=[[decision]];wb.recalculate();
assert.equal(summary.getRange('E15').values[0][0],80);
assert.deepEqual(summary.getRange('B7:B12').values.flat(),[0,0,0,0,0,0]);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='Pending').length,10);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='AI full-text: include').length,79);
...
assert.equal(batch.records.filter(r=>r.local_pdf_acquired).length,9);
...
for(const [name,range] of [['Summary','A5:H29'],['Screening Log','I47:N47'],['Evidence Matrix','O33:X33'],['Review Protocol','A21:B25']]){
