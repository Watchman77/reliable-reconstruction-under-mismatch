// One-time v0.11 -> v0.12 migration; preserves seed IDs, historic triage and formal decisions.
...
if(!repo||!out)throw new Error('Usage: node update_full_text_05.mjs REPO OUTPUT_DIR');
await fs.mkdir(out,{recursive:true});
const batch=JSON.parse(await fs.readFile(path.join(repo,'literature/screening/full_text_05.json'),'utf8'));
const input=path.join(repo,'literature/evidence_matrix_2020_2026.xlsx');
const bytes=await fs.readFile(input);
const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
assert.equal(sha,'681be3d836e399e1459ace9be566854ed92cf45d','Requires v0.11; must not overwrite a later checkpoint.');
...
assert.equal(refs.length,90); assert.equal(batch.records.length,9);
const snapshots=[];
...
for(const r of batch.records){
 ...
 ref['Review notes']+=` | AI full-text ${batch.checked_on}: ${r.decision}; human pending. Verified fields: ${Object.keys(r.feature_codes).join(', ')}. Evidence/version/read extent: screening/full_text_05.json (${r.study_id}). Unlisted feature codes remain provisional.`;
 ...
 set(log,`L${row}`,`${batch.checked_on} | AI full-text ${r.decision}; human pending. ${r.decision==='exclude'?r.exclusion_detail:r.criteria['3'].evidence} Online text inspected; no local PDF saved. Version, read extent and technical extraction: screening/full_text_05.json (${r.study_id}).`);
...
set(summary,'A17','Unknown-degradation prompts, compound restoration, operator conditioning and image posterior sampling have existing comparators. See screening/full_text_05_report.md for assumptions and version-specific limits. Earlier checkpoints remain available. Novelty is unestablished.');
set(protocol,'B22','Initial triage 90/90 (82 advance, 8 unclear). AI full-text assessed: 53 (52 include, 1 exclude), 37 remain. All human adjudication and formal searches/inclusion pending. Formal inclusion/exclusion counts remain zero.');
set(protocol,'B23','90 seed records retained. Cumulative full-text progress: screening/full_text_01.json, full_text_02.json, full_text_03.json, full_text_04.json and full_text_05.json; current report: full_text_05_report.md. AI decisions require human adjudication. Only listed feature fields are verified; other codes remain provisional.');
wb.recalculate();
assert.deepEqual(summary.getRange('E11:E14').values.flat(),[90,82,8,0]);
assert.equal(summary.getRange('E15').values[0][0],53);
const decision=log.getRange('J13').values[0][0];log.getRange('J13').values=[['Pending']];
assert.equal(summary.getRange('E15').values[0][0],52);
log.getRange('J13').values=[[decision]];wb.recalculate();
assert.equal(summary.getRange('E15').values[0][0],53);
assert.deepEqual(summary.getRange('B7:B12').values.flat(),[0,0,0,0,0,0]);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='Pending').length,37);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='AI full-text: include').length,52);
...
for(const [name,range] of [['Summary','A5:H29'],['Screening Log','I13:N13'],['Evidence Matrix','O80:X80'],['Review Protocol','A21:B25']]){
