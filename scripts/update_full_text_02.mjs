// One-time v0.8 -> v0.9 migration; preserves seed IDs, historic triage and formal decisions.
...
if(!repo||!out)throw new Error('Usage: node update_full_text_02.mjs REPO OUTPUT_DIR');
await fs.mkdir(out,{recursive:true});
const batch=JSON.parse(await fs.readFile(path.join(repo,'literature/screening/full_text_02.json'),'utf8'));
const input=path.join(repo,'literature/evidence_matrix_2020_2026.xlsx');
const bytes=await fs.readFile(input);
const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
assert.equal(sha,'ba0fae031e3f57ef0d6a8fd1968f8eb1358f2cb9','Requires v0.8; must not overwrite a later checkpoint.');
...
for(const r of batch.records){
 ...
 ref['Review notes']+=` | AI full-text ${batch.checked_on}: ${r.decision}; human pending. Verified fields: ${Object.keys(r.feature_codes).join(', ')}. Evidence/version/read extent: screening/full_text_02.json (${r.study_id}). Unlisted feature codes remain provisional.`;
 ...
 set(log,`L${row}`,`${batch.checked_on} | AI full-text ${r.decision}; human pending. ${r.decision==='exclude'?r.exclusion_detail:r.criteria['3'].evidence} Online text inspected; no local PDF saved. Version, read extent and technical extraction: screening/full_text_02.json (${r.study_id}).`);
...
set(summary,'A17','New comparisons cover operator point estimation, prior shift, compound restoration and calibrated image/task uncertainty. Read assumptions and evidence limits in screening/full_text_02_report.md; prior findings remain in full_text_01. Novelty is unestablished.');
set(protocol,'B22','Initial triage 90/90 (82 advance, 8 unclear). AI full-text assessed: 24 (23 include, 1 exclude), 66 remain. All human adjudication and formal searches/inclusion pending. Formal inclusion/exclusion counts remain zero.');
set(protocol,'B23','90 seed records retained. Cumulative full-text progress: screening/full_text_01.json and full_text_02.json; current report: full_text_02_report.md. AI decisions require human adjudication. Only listed feature fields are verified; other codes remain provisional.');
wb.recalculate();
assert.deepEqual(summary.getRange('E11:E14').values.flat(),[90,82,8,0]);
assert.equal(summary.getRange('E15').values[0][0],24);
const decision=log.getRange('J27').values[0][0];log.getRange('J27').values=[['Pending']];
assert.equal(summary.getRange('E15').values[0][0],23);
log.getRange('J27').values=[[decision]];wb.recalculate();
assert.equal(summary.getRange('E15').values[0][0],24);
assert.deepEqual(summary.getRange('B7:B12').values.flat(),[0,0,0,0,0,0]);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='Pending').length,66);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='AI full-text: include').length,23);
...
for(const [name,range] of [['Summary','A5:H29'],['Screening Log','I27:N27'],['Evidence Matrix','O37:X37'],['Review Protocol','A21:B23']]){
