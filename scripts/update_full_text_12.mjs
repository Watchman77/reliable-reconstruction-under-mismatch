// One-time checkpoint 11 -> checkpoint 12 migration; preserves seed IDs, historic triage and formal decisions.
...
if(!repo||!out)throw new Error('Usage: node update_full_text_12.mjs REPO OUTPUT_DIR');
await fs.mkdir(out,{recursive:true});
const batch=JSON.parse(await fs.readFile(path.join(repo,'literature/screening/full_text_12.json'),'utf8'));
const input=path.join(repo,'literature/evidence_matrix_2020_2026.xlsx');
const bytes=await fs.readFile(input);
const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
assert.equal(sha,'fbfb11d60961d0672cc361e5267d8c5fe6be671a','Requires checkpoint 11; must not overwrite a later checkpoint.');
...
assert.equal(refs.length,90); assert.equal(batch.records.length,1);
const snapshots=[];
...
for(const r of batch.records){
 ...
 for(const [col,value] of Object.entries(r.matrix_updates??{})) set(matrix,`${col}${row}`,value,`${r.study_id}: verified from supplied article; ${r.feature_locator}`);
 Object.assign(ref,r.reference_updates??{});
 const status='AI full-text eligibility assessed; selected features verified';
 ref['Evidence status']=status;
 ref['Review notes']+=` | AI full-text ${batch.checked_on}: ${r.decision}; human pending. Verified fields: ${Object.keys(r.feature_codes).join(', ')}. Evidence/version/read extent: screening/full_text_12.json (${r.study_id}). Unlisted feature codes remain provisional.`;
...
for(const r of batch.pending_access){
 ...
 assert(i>=0);assert.equal(matrix.getRange(`A${row}`).values[0][0],r.study_id);
 assert.equal(log.getRange(`J${row}`).values[0][0],'Pending');
 ref['Review notes']+=` | Retrieval ${batch.checked_on}: full-text eligibility pending. Source attempts and next action: screening/full_text_12.json (${r.study_id}, pending_access). No new feature verification.`;
 set(matrix,`AF${row}`,ref['Review notes'],'Document access attempts; eligibility unchanged.');
 set(log,`L${row}`,`${batch.checked_on} | Full-text eligibility remains pending. ${r.attempts[0].result} ${r.next_action} Complete retrieval log: screening/full_text_12.json (${r.study_id}).`, 'Document unresolved access without an eligibility decision.');
 log.getRange(`A${row}:N${row}`).format.rowHeight=144;
 log.getRange(`J${row}:N${row}`).format.wrapText=true;
 log.getRange(`A${row}:N${row}`).format.verticalAlignment='top';
}
for(const item of batch.lineage_updates??[]){
 for(const id of item.study_ids){
  ...
  ref['Review notes']+=` | Lineage ${batch.checked_on}: P053 explicitly identifies P026 as previous work and comparator. Relationship confirmed; study-level grouping pending. See screening/full_text_12.json (lineage_updates).`;
  set(matrix,`AF${row}`,ref['Review notes'],'Link predecessor evidence without changing formal study grouping or decisions.');
 }
}
set(summary,'A17','P035 adds learned residual correction and an empirical failure threshold. Its true-model discrepancy is not a calibrated score for unknown captures. Test observable reliability scores against strong controls. See screening/full_text_12_report.md. Novelty remains unestablished.');
set(protocol,'B22','Initial triage 90/90 (82 advance, 8 unclear). AI full-text assessed: 90 (89 include, 1 exclude), 0 remain. Human adjudication and formal searches/inclusion pending. Formal inclusion/exclusion counts remain zero.');
set(protocol,'B23','90 seed records retained. Cumulative evidence: screening/full_text_01.json through full_text_12.json. Current report: full_text_12_report.md. AI decisions require human adjudication. Only listed feature fields are verified; other codes remain provisional.');
wb.recalculate();
assert.deepEqual(summary.getRange('E11:E14').values.flat(),[90,82,8,0]);
assert.equal(summary.getRange('E15').values[0][0],90);
const decision=log.getRange('J36').values[0][0];log.getRange('J36').values=[['Pending']];
assert.equal(summary.getRange('E15').values[0][0],89);
assert(summary.getRange('A22').values[0][0].includes('1 remains.'));
log.getRange('J36').values=[[decision]];wb.recalculate();
assert.equal(summary.getRange('E15').values[0][0],90);
assert(summary.getRange('A22').values[0][0].includes('0 remain.'));
assert.deepEqual(summary.getRange('B7:B12').values.flat(),[0,0,0,0,0,0]);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='Pending').length,0);
assert.equal(log.getRange('J2:J91').values.filter(r=>r[0]==='AI full-text: include').length,89);
...
assert.equal(batch.records.filter(r=>r.local_pdf_acquired).length,1);
...
for(const [name,range] of [['Summary','A5:H29'],['Screening Log','I36:N36'],['Evidence Matrix','O36:X36'],['Review Protocol','A21:B25']]){
