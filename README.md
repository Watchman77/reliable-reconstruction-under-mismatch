...
- [JPEG-aware baseline 04](experiments/jpeg_aware_04/README.md) now has a [reviewed four-source Colab run](experiments/jpeg_aware_04/colab_execution_review_2026-09-17.md): all 23 cell sources match, 112 quality rows and 896 risk rows are recorded, and both frozen descriptive development screens are independently reproduced from CSVs. FBCNN + DPIR gains 0.890 dB and lowers detail MSE by 31.810% versus raw DPIR with JPEG; without JPEG it loses 0.084 dB. Operator detail spread lowers primary retained detail error by 10.920% versus whole-pipeline rotation spread, winning on four sources. All 14 CSVs, seven JSONs and four inspected embedded figures match 25 of 37 export-manifest entries. Eight regenerated observation hashes and 16 independently recomputed input/classical rows agree. Full raw-array/ZIP verification remains pending after HTTP 403 on a streamed part download; all six existing parts are needed. Trained uncertainty comparisons, stronger-baseline checks, calibration, independent testing and novelty remain outstanding.
