...
- [Baseline 01](experiments/baseline_01/README.md) now has a saved, executed development notebook, result tables and four inspected figures. Across 48 simulated combinations of the same four sources, source-excluded tuning gives the nominal gradient-regularised inverse pooled gains of 1.141 dB and 0.615 dB over input for linear and JPEG-chain conditions. Ridge remains weaker on average. Patch-level operator spread improves over the included controls in pooled comparisons; these are development findings, not novelty or independent-test claims. The initial execution used in-process IPython. The [subsequent saved Colab run is now reviewed](experiments/baseline_01/colab_execution_review_2026-09-17.md), including eleven code cells, three reconciled tables and four embedded figures. Complete export-file hash verification remains outstanding.

- [Learned baseline 02](experiments/learned_02/README.md) now has a self-contained DRUNet/DPIR-style notebook and one executed full-resolution development case (0801, linear blur/noise). Strict official weight/source checks pass. Nominal learned reconstruction scores 34.581 dB versus 33.954 dB for the classical inverse and 32.583 dB for input. Equal-denoiser-call operator and image-transformation sensitivity comparisons are recorded, with positive detail-selection comparisons on this case. All 16 export hashes and saved prediction/metric readback pass; four figures are inspected. The final two presentation cells were replayed from saved results after a notebook-saving discrepancy, with explicit metadata. The default eight-observation Colab run, GPU validation and independent evaluation remain pending; training overlap precludes unseen-image claims.
