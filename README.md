...
- [JPEG-aware baseline 04](experiments/jpeg_aware_04/README.md) implements the bounded follow-up with official blind FBCNN preprocessing, six main reconstruction comparisons and operator/rotation selection controls. The [one-source local validation](experiments/jpeg_aware_04/local_execution_review_2026-09-17.md) completed both conditions and independently reconciled all 28 quality rows, 56 patch-error vectors, 32 score vectors and 224 risk rows. On 0801, FBCNN + DPIR gains 1.528 dB over raw DPIR with JPEG but loses 0.170 dB without JPEG; its operator-detail ranking beats the included primary selection controls. These are development observations only. The clean Colab notebook defaults to all four sources and automatically exports a summary plus verified RAW transfer parts. Full-design screens, GPU/Drive execution, trained uncertainty comparison, calibration, independent testing and novelty remain outstanding.
