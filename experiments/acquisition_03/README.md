...
**Latest checkpoint:** the [complete raw archive audit passed](archive_verification_20260917/verification.md). All five transfer parts reproduce the original archive hash; all 38 exported file hashes, all 80 saved reconstruction metric rows, all 160 patch-error vectors and all 240 full-coverage tail rates agree. Four original source images were rehashed. Nine CSVs and four raw figures are byte-identical to the [earlier Colab review](colab_execution_review_2026-09-17.md). The four figures and a new fixed-inset montage covering all four sources were inspected. No further upload or experiment rerun is needed for this verification.

The four-source result remains unchanged: nominal DPIR's advantage persists through clipping/8-bit rounding and reverses overall at the JPEG step. Its JPEG detail ranking loses on all four sources; its RGB ranking loses on three of four. This is development evidence, not novelty or held-out validation.

**Transfer history:** [Notebook 03A](../../notebooks/03A_Package_Existing_Acquisition_Results.ipynb) resolved the oversized-archive transfer by preserving its bytes in five smaller ZIPs. The [access record](archive_transfer_access_2026-09-17.md) and [local helper validation](transfer_helper_validation_2026-09-17.json) remain available for reproducibility. The current run's raw-verification gap is closed.
